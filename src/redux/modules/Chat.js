...
const leaveRoomDB = (roomId) => {
  return async function (dispatch, getState, { history }) {
    // console.log("leaveRoomDB : roomId", roomId)
    ChatAPI.leaveRoom(roomId)
    .then((response) => {
      // console.log("leaveRoom : response", response);
      window.alert("방에서 퇴장하였습니다.")
      history.replace('/chat/1')
      dispatch(getChatRoomDB())
    }).catch((error) => {
      console.log("leaveRoomDB : error", error.response);
    })
...
export default handleActions(
  {
    [GET_CHAT_ROOM]: (state, action) => produce(state, (draft) => {
      ...
      draft.list = action.payload.chat_list.reverse();
    }),
